...
# problem 2
library(readxl)
glass <- read_excel("/Users/zhangyueming/Documents/NEU/2017Fall/IE7275/HW/HW2/Glass Identification Data.xlsx")
fa.parallel(glass[, 2:10], fa = "pc", n.iter = 100,
            show.legend = FALSE, main = "Scree plot with parallel analysis")
abline(h = 1, lwd = 1, col = "green")
pc <- principal(glass[, 2:10], nfactors = 4, rotate = "none")
pc

rc <- principal(glass[, 2:10], nfactors = 4, rotate = "varimax")
rc

pc <- principal(glass[, 2:10], nfactors = 4, scores = TRUE)
head(pc$scores)
...
fa.parallel(correlations, n.obs = 305, fa = "both", n.iter = 100,
...
fa.24tests <- fa(Harman74.cor$cov, nfactors = 4, rotate = "none")
fa.24tests

fa.24tests$weights

factor.plot(fa.24tests)

fa.diagram(fa.24tests)



# problem 5
vertebral <- read_excel("/Users/zhangyueming/Documents/NEU/2017Fall/IE7275/HW/HW2/Vertebral Column Data.xlsx")
fa.parallel(vertebral[, -7], fa = "pc", n.iter = 100,
            show.legend = FALSE, main = "Scree plot with parallel analysis")
abline(h = 1, lwd = 1, col = "green")
d <- dist(vertebral[, -7])
fit <- cmdscale(d) # k is the number of dim
fit # view results
x<-fit[,1]
y<-fit[,2]
# plot solution 
...
text(x, y, labels = row.names(vertebral[, -7]), cex=.7)
