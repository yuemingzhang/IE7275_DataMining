...
# In the IMDB data, we have some duplicate rows.
# We want to remove the 45 duplicated rows and keep the unique ones.
...
# First, we want to know if genre is related to imdb score. We divide the string into several substrings by the separator '|', and save each substring along with its correspongding imdb score in a different csv file. Then we plot a histogram for the score and genres to see if they are relative.
...
# There isn't much difference in the averages of imdb score related to different genres, almost all the averages are in the range of 6~8. So we think the predictor "genre" can be removed because it's not really related to the score.
...
# Now let's see how many complete cases do we have.
...
# Now aspect_ratio has the highest number of missing values.
# Before trying to impute the missing values, we want to check how important is this variable.
...
# So, removing this variable won't affect our following analysis.
...
# Now we finished imputing the numeric missing values. The remaining missing values are characters, let's keep them there.
...
# Around 79% movies are from USA, 8% from UK, 13% from other countries. So we bin this variable into three buckets: USA, UK, Others.
...
## 4.5 Commercial Success vs Critical Acclaim
IMDB %>%
  top_n(20, profit) %>%
...
# We have 2399 directors, and 4884 actors in this data.
...
# For the purpose of data exploration, we added two variables based on existing variables: profit and return_on_investment_perc.
# In order to avoid multicollinearity, here we remove these two added variables.

IMDB <- subset(IMDB, select = -c(profit, return_on_investment_perc))

## 5.3 Remove high correlation variables

# First we plot the correlation heatmap for our data.

ggcorr(IMDB, label = TRUE, label_round = 2, label_size = 3.5, size = 3, hjust = .85) +
...
tree.pred <- predict(pruned.ct, valid, type = "class")
# generate confusion matrix for training data
confusionMatrix(tree.pred, valid$binned_score)
...
# The accuracy on validation data is 0.7642.
...
# The accuracy on test data is 0.7658.
